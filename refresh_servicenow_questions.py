...
BASE_URL = "https://www.examtopics.com/exams/servicenow/"
...
def extract_servicenow_exams() -> list[dict]:
    list_html = fetch_text(BASE_URL)
    links = re.findall(
        r'href="(/exams/servicenow/[^"]+)"[^>]*class="popular-exam-link"[^>]*>\s*(.*?)\s*</a>',
        list_html,
        re.S,
    )
...
def update_json() -> None:
    ...
    for provider in data:
        if provider.get("provider") == "ServiceNow":
            provider["exams"] = extract_servicenow_exams()
            break

    file_path.write_text(json.dumps(data, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(f"ServiceNow questions refreshed in {file_path.name}")
